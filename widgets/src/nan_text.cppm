                ...
                for (std::size_t i = 0; i <= text.size(); ++i) {
                        if (i == text.size() || is_text_break(text[i])) {
                                if (i > run_begin) {
                                    widest = std::max(
                                        widest,
                                        font.estimate_text_width(
                                            text.substr(run_begin, i - run_begin)
                                        )
                                    );
                                }
                            run_begin = i + 1;
    ...
    public:
        ...
        virtual auto set_style(const TextStyle& style) -> Text& {
            ...
            bool draw_dirty = false;
                ...
                if (style.wrap_policy.has_value()
                    && style.wrap_policy.value() != m_font.wrap_policy())
                {
                    m_font.wrap_policy(style.wrap_policy.value());
                    layout_dirty = true;
                }
                if (style.single_line.has_value()
                    && style.single_line.value() != m_font.single_line())
                {
        ...
        [[nodiscard]] auto typography_role() const noexcept
            -> std::optional<theme::NanTypographyRole> {
        ...
        [[nodiscard]] auto preferred_size() const noexcept -> geometry::NanSize override {
            ...
            // Use the same layout engine as measure() for consistency.
            // ensure_shaped() triggers font loading on first call.
            ensure_shaped();
                if (!m_shaped_glyphs.empty()) {
                    const auto& layout = layout_at(0.0f);
                        if (!layout.empty() && layout.total_width > 0.0f) {
                            return {layout.total_width, layout.total_height};
                        }
                }

            // Fallback: layout returned empty (e.g. whitespace-only text)
                if (m_font.is_loaded()) {
                    const float text_w = m_font.estimate_text_width(txt);
                    return {text_w, m_font.line_height()};
                }

            // Font not yet loaded: rough estimate
            const float fs = m_font.size();
            const float text_w = static_cast<float>(txt.size()) * fs * 0.6f;
            return {text_w, fs * 1.4f};
        }

        [[nodiscard]] auto layout_info(const bool for_width, const float cross_constraint) const
            -> types::LayoutInfo override {
            const auto txt = display_text();
            if (txt.empty())
                return {};
        ...
        auto measure(const geometry::NanConstraints& constraints) -> void override {
            ...
            set_measured_layout_state(
                ...
                constraints.constrain(
                    measured.width() <= 0.0f && measured.height() <= 0.0f ? preferred_size()
                                                                          : measured
                )
            );
        }
    ...
    protected:
        ...
        void on_draw(tvg::SwCanvas& canvas) override {
            ...
            float max_width = bnds.width();
                if (max_width < 1.0f && avail_w > 0.0f
                    && avail_w < geometry::NanConstraints::k_infinity)
                {
                    max_width = avail_w;
                }
            ...
            // 与父级 clip stack 取交集（如果有），提供更紧的 containment
            geometry::NanRect clip_rect = bnds;
                if (const auto active_clip = nandina::runtime::active_draw_clip()) {
                    const auto intersected = clip_rect.intersected(active_clip->rect);
                        if (intersected.width() > 0.0f && intersected.height() > 0.0f) {
            ...
            text_font.paint(canvas, layout, offset_x, offset_y, &clip_rect);
        }
