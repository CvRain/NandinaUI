...
namespace detail {
    ...
    elapsed_ms(const SteadyClock::time_point start, const SteadyClock::time_point end) noexcept
        -> double {
...
public:
    ...
    auto bind(this Self&& self, Ref<T>& ref) -> Self&& {
        ...
        node.m_ref_binders.push_back([&ref, widget]() { ref.bind(widget); });
        node.m_ref_resetters.push_back([&ref]() { ref.reset(); });
...
public:
    ...
    template<typename Self>
        requires std::derived_from<std::remove_cvref_t<Self>, LabelNode>
    auto typography_role(this Self&& self, const nandina::theme::NanTypographyRole value)
        -> Self&& {
    ...
    template<typename Self>
        requires std::derived_from<std::remove_cvref_t<Self>, LabelNode>
    auto style(this Self&& self) -> nandina::widgets::TextStyle {
        return self.m_typed->get_style();
    }
    ...
    [[nodiscard]] static auto positioned_container(Children children) -> Node {
        ...
        for (auto& child: std::move(children).take()) {
            ...
            if (auto cw = std::move(child).take_widget()) {
                static_cast<W*>(result.widget_ptr())
                    ->add_positioned_child(std::move(cw), std::move(props));
            }
...
[[nodiscard]] inline auto expanded(NodeLike auto&& child) -> Node {
    return NodeFactory::wrapper_with<nandina::layout::Expanded>(
...
protected:
    ...
    [[nodiscard]] auto hit_test_root_component(const float x, const float y)
        -> runtime::NanWidget* {
        if (!m_root_component) {
            return nullptr;
        }
...
private:
    ...
    auto sync_hover_target(runtime::NanWidget* next, const runtime::PointerMoveEvent& event)
        -> bool {
    ...
    auto apply_throttled_resize() -> void {
        ...
        const auto elapsed_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(now - m_last_resize_apply_time)
                .count();
    ...
    protected:
        ...
        void on_pointer_up(const runtime::PointerButtonEvent& event) override {
            ...
            m_owner.sync_hover_target(hit, pointer_move_from_button(event));
            auto* target =
                m_owner.m_pointer_capture_widget ? m_owner.m_pointer_capture_widget : hit;
            if (target) {
        ...
        void on_text_input(std::string_view text) override {
            if (m_owner.m_focused_widget && !text.empty()) {
                m_owner.m_focused_widget->dispatch_event(
                    runtime::TextInputEvent { .text = std::string { text } }
                );
            }
        }

        void
        on_text_editing(std::string_view text, std::int32_t start, std::int32_t length) override {
            if (m_owner.m_focused_widget) {
                m_owner.m_focused_widget->dispatch_event(
...
inline auto NanAppWindow::run() -> void {
    ...
    runtime::NanWindow::Config runtime_config { .title = m_config.title,
