...
protected:
    auto on_draw(tvg::SwCanvas &canvas) -> void override {
        ...
        // ─ 2. 侧边栏 — 左侧导航 + 项目列表 ──────────
        const float sidebar_w = 240.0f;
        draw_sidebar(canvas, sidebar_w, h);

        // ─ 3. 底部 Dock 栏 ────────────────────────────
        const float dock_h = 56.0f;
        draw_dock(canvas, sidebar_w, h - dock_h, w - sidebar_w, dock_h);

        // ─ 4. 主内容区域 ──────────────────────────────
        draw_main_content(canvas, sidebar_w, 0.0f, w - sidebar_w, h - dock_h);

        // ─ 5. 动画计数 ──────────────────────────────
        tick_count += 1;
        if (tick_count % 60 == 0) {
            status_pulse = (status_pulse + 0.05f);
            if (status_pulse > 1.0f) status_pulse -= 1.0f;
        }
...
private:
    ...
    void draw_sidebar(tvg::SwCanvas &canvas, float w, float h) const {
        ...
        draw_line(canvas, 0, 0, w, 0, 60, 62, 82, 255, 1);
        ...
        draw_text_dots(canvas, 55, logo_y + 8, "Nandina Studio", text_primary.r, text_primary.g, text_primary.b, 200, 11);
        ...
        draw_text_dots(canvas, 26, proj_y + 3, "Recent Projects", text_secondary.r, text_secondary.g, text_secondary.b, 180, 9);
    ...
    void draw_dock(tvg::SwCanvas &canvas, float x, float y, float w, float h) const {
        ...
        draw_text_dots(canvas, x + w - 60, y + h * 0.5f - 4, "10:42", text_secondary.r, text_secondary.g, text_secondary.b, 180, 9);
    }

    // ── 主内容区 ────────────────────────────────────
    void draw_main_content(tvg::SwCanvas &canvas, float x, float y, float w, float /* h */) const {
        using namespace nandina::layout;
        ...
        draw_text_dots(canvas, x + 20, y + 16, "Dashboard / Overview",
                       text_secondary.r, text_secondary.g, text_secondary.b, 200, 10);

        draw_rect(canvas, x + w - 100, y + 10, 28, 24, 55, 57, 75, 200, 4);
        draw_rect(canvas, x + w - 66, y + 10, 28, 24, 55, 57, 75, 200, 4);
        draw_rect(canvas, x + w - 38, y + 6, 32, 32, accent.r, accent.g, accent.b, 200, 6);
        draw_text_dots(canvas, x + w - 28, y + 15, "+", 255, 255, 255, 220, 12);
        ...
        // ─ 第一行：卡片行 ───────────────
        {
            ...
            row1.container_bounds = {x + 20, content_top, x + w - 20, content_top + 100.0f};
            row1.gap = card_gap;
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                child.preferred_size = {card1_w, 0.0f};
                child.flex_factor = 1;
                row1.children.push_back(child);
            }

            auto frames1 = backend.compute(row1);

            const auto card_titles = std::to_array<std::string_view>({
                "Total Users", "Active Now", "Revenue", "Tasks"
            });
            const auto card_values = std::to_array<std::string_view>({
                "2,847", "143", "$12.4k", "18/24"
            });
            const auto card_colors = std::to_array<std::tuple<uint8_t, uint8_t, uint8_t>>({
                std::tuple{99, 102, 241},
                std::tuple{95, 200, 130},
                std::tuple{245, 158, 60},
                std::tuple{236, 110, 130},
            ...
            for (size_t i = 0; i < frames1.size() && i < card_titles.size(); ++i) {
                const auto &f = frames1[i];
                const auto &[cr, cg, cb] = card_colors[i];

                draw_rect(canvas, f.x(), f.y(), f.width(), f.height(), card_bg.r, card_bg.g, card_bg.b, 255, 8);
                draw_rect(canvas, f.x() + 8, f.y() + 8, f.width() - 16, 3, cr, cg, cb, 220, 1.5f);

                draw_text_dots(canvas, f.x() + 14, f.y() + 22, card_titles[i],
                               text_dim.r, text_dim.g, text_dim.b, 180, 8);
                draw_text_dots(canvas, f.x() + 14, f.y() + 42, card_values[i],
                               text_primary.r, text_primary.g, text_primary.b, 220, 14);

                const bool up = (i % 2 == 0);
                auto *indicator = tvg::Shape::gen();
                const float ind_x = f.x() + 14;
                const float ind_y = f.y() + 64;
                if (up) {
                    indicator->moveTo(ind_x + 4, ind_y + 6);
                    indicator->lineTo(ind_x, ind_y);
                    indicator->lineTo(ind_x + 8, ind_y);
                    indicator->fill(green.r, green.g, green.b, 200);
                } else {
                    indicator->moveTo(ind_x + 4, ind_y);
                    indicator->lineTo(ind_x, ind_y + 6);
                    indicator->lineTo(ind_x + 8, ind_y + 6);
                    indicator->fill(pink.r, pink.g, pink.b, 200);
                }
                canvas.add(indicator);

                const auto &ind_color = up ? green : pink;
                draw_text_dots(canvas, ind_x + 14, ind_y + 1,
                               up ? "+12.5%" : "-3.2%",
                               ind_color.r, ind_color.g, ind_color.b, 200, 7);
            }
        }
        ...
        // ─ 第二行：左侧图表 + 右侧活动列表 ──
        {
            ...
            for (size_t ai = 0; ai < activities.size(); ++ai) {
                ...
                const float pulse_alpha = 150.0f + 70.0f * (0.5f + 0.5f * std::sin(static_cast<float>(tick_count) * 0.03f + pulse_offset));
        ...
        // ─ 第三行：Stack 展示 + 进度卡片 ──
        {
            ...
            // ─ 左侧：Stack 重叠演示 ──────────
            {
                ...
                draw_rect(canvas, f.x(), f.y(), f.width(), f.height(), card_bg.r, card_bg.g, card_bg.b, 255, 8);
                draw_text_dots(canvas, f.x() + 14, f.y() + 14, "Stack Layout Demo",
                               text_primary.r, text_primary.g, text_primary.b, 220, 9);

                // 用 Stack 布局演示重叠
                LayoutRequest stack_req;
                stack_req.axis = LayoutAxis::stack;
                stack_req.container_bounds = {f.x() + 20, f.y() + 30, f.x() + f.width() - 20, f.y() + f.height() - 16};
                ...
                auto stack_frames = backend.compute(stack_req);
                const auto stack_colors = std::to_array<std::tuple<uint8_t, uint8_t, uint8_t, uint8_t>>({
                    std::tuple{99, 102, 241, 180},
                    std::tuple{147, 150, 255, 200},
                    std::tuple{200, 200, 255, 220},
                });

                for (size_t si = 0; si < stack_frames.size() && si < stack_colors.size(); ++si) {
                    const auto &sf = stack_frames[si];
                    const auto &[sr, sg, sb, sa] = stack_colors[si];
                    draw_rect(canvas, sf.x(), sf.y(), sf.width(), sf.height(), sr, sg, sb, sa, 8);
                }

                draw_text_dots(canvas, f.x() + 14, f.y() + f.height() - 14,
                               "Column · Row · Stack — all from BasicLayoutBackend",
                               text_dim.r, text_dim.g, text_dim.b, 120, 6);
            }
            ...
            // ─ 右侧：进度卡片 ──────────────
            {
                ...
                const auto progs = std::to_array<ProgressItem>({
                ...
                for (size_t pi = 0; pi < progs.size(); ++pi) {
                    const auto &p = progs[pi];
                    const float py = f.y() + 34.0f + static_cast<float>(pi) * 18.0f;

                    draw_text_dots(canvas, bar_x, py, p.label, text_secondary.r, text_secondary.g, text_secondary.b, 160, 7);

                    const float pbar_x = f.x() + f.width() * 0.5f;
                    const float pbar_w = f.width() * 0.42f;
                    draw_rect(canvas, pbar_x, py + 1, pbar_w, 6, 42, 44, 62, 200, 3);
                    draw_rect(canvas, pbar_x, py + 1, pbar_w * p.pct, 6, p.r, p.g, p.b, 200, 3);

                    char pct_str[8];
                    const int len = std::snprintf(pct_str, sizeof(pct_str), "%d%%", static_cast<int>(p.pct * 100.0f));
                    if (len > 0) {
                        draw_text_dots(canvas, pbar_x + pbar_w + 8, py,
                                       std::string_view{pct_str, static_cast<size_t>(len)},
                                       text_dim.r, text_dim.g, text_dim.b, 150, 7);
                    }
                }
            }
        }
        ...
        // ─ 底部分隔线 ──────────────────────────
        const float footer_y = content_top + 116.0f + 200.0f + 20.0f + 110.0f + 20.0f;
        draw_line(canvas, x + 20, footer_y, x + w - 20, footer_y, 55, 57, 75, 100);
        draw_text_dots(canvas, x + 20, footer_y + 12,
                       "Built with NandinaUI · ThorVG rendering · C++26 modules",
                       text_dim.r, text_dim.g, text_dim.b, 100, 7);
    }
...
private:
    ...
    mutable int tick_count = 0;
    mutable float status_pulse = 0.0f;
};
