...
> C ABI、Zig/C++ 双前端、showcase 可视化画廊已完成；多页导航（Page/Router）与全包窗口入口已可用。
...
> 当前默认后端为 software（纯 Zig 软件光栅），ThorVG 暂不渲染文字。
> HarfBuzz 链接但未启用 text shaping（kerning / 复杂文字待办）。
> 详见 [`docs/development/showcase-refactor-plan.md` §11](docs/development/showcase-refactor-plan.md#11-现状缺口盘点字体渲染--渲染后端--组件库)
