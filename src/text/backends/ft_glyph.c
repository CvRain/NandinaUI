...
NandinaFTGlyphResult nandina_ft_render_glyph(FT_Face face,
  ...
  result.advance_x =
      (int)(slot->advance.x >> 6); // Convert from 26.6 fixed point to pixels
  return result;
}
...
// Compatibility stub: vcpkg FreeType was compiled on glibc 2.43, which uses
// __isoc23_strtol (added in glibc 2.38).  Zig's LLD doesn't have this symbol,
// so we provide a weak alias to strtol.
