...
> 状态校正（2026-05）：本文件描述“当前已经成立的主线分层 + 仍然有效的目标边界”，不再是从零立项时的纯目标态说明。
>
> 当前现状：`foundation`、`runtime`、`reactive`、`layout`、`theme`、`text`、`widgets`、`app`、`showcase` 均已有实际代码；`render` 正式抽象层与 `bindings` 仍偏早期。
...
### render
渲染抽象层与后端适配接口。当前正式抽象层仍未落地，ThorVG 仍直接在 runtime/widgets/showcase 中承担绘制职责。
...
其中 app authoring 与 widgets 的语义 API 边界，当前特别需要稳定以下约束：

- 业务层不再手写坐标与 child 遍历
- 公共组件组合 API 不再要求使用者显式处理 `std::move`
- 根组件挂载应收敛到统一入口
- 挂载后的组件访问应通过 `Ref / Handle / Key` 之类的句柄机制完成，而不是依赖所有权变量
...
> 注：以上边界里，app / widgets / layout / reactive 的主路径已经进入“收口”阶段；真正仍处于早期探索的主要是 render abstraction、bindings 与更后续的脚本层能力。
