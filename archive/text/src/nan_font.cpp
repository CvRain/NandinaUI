...
namespace nandina::text
{
    ...
    namespace
    {
        ...
        struct FontCacheKeyHash {
            auto operator()(const FontCacheKey& key) const noexcept -> std::size_t {
                ...
                return path_hash
                    ^ (size_hash + 0x9e3779b9u + (path_hash << 6u) + (path_hash >> 2u));
            }
        };
        ...
        auto font_cache_key(std::string_view path, float size_pt) -> FontCacheKey {
            return FontCacheKey {
                ...
                .size_centipt =
                    static_cast<int>(size_pt * 100.0f + (size_pt >= 0.0f ? 0.5f : -0.5f)),
            };
        }
    ...
    auto NanFont::Impl::init_from_face(FT_Face face, float size_pt_val) -> void {
        ...
        const float raw_gap =
            static_cast<float>(metrics.height - metrics.ascender + metrics.descender) / 64.0f;
    ...
    auto NanFont::Impl::ensure_fallbacks() -> void {
            for (const auto& fb_path: fallback_paths) {
                ...
                FT_Set_Char_Size(
                    fb_face,
                    0,
                    static_cast<FT_F26Dot6>(size_pt * 64.0f + 0.5f),
                    0,
                    96
                );

                    // 检查 metrics 是否有效；对 TTC 尝试后续 face_index
                    if (fb_face->size->metrics.ascender == 0
                        && fb_face->size->metrics.descender == 0)
                    {
    ...
    auto NanFont::load_from_path(std::string_view file_path, float size_pt) -> NanFont {
        ...
        const FT_Error error = FT_New_Face(global_ft().library, path_str.c_str(), 0, &face);
            if (error != FT_Err_Ok || !face) {
                ...
                throw std::runtime_error(
                    std::format("FT_New_Face failed for \"{}\": error={}", path_str, error)
                );
            }
                    ...
                    if (font.m_impl->faces.empty()) {
                        FT_Error alt_err = FT_Err_Ok;
                            for (int fi = 1; fi <= 3 && font.m_impl->faces.empty(); ++fi) {
                                ...
                                if (alt_err != FT_Err_Ok || !alt_face)
                                    continue;
                                font.m_impl->init_from_face(alt_face, size_pt);
                            }
                    }
                font.m_impl->loaded = true;
                font.m_impl->fallback_paths = NanFont::Impl::system_fallback_paths();
            }
            catch (...) {
                FT_Done_Face(face);
                throw;
        ...
        log.info(
            "Font loaded: \"{}\" {:.1f}pt (pixel={:.2f})",
            path_str,
            size_pt,
            font.m_impl->pixel_height
        );
    ...
    auto NanFont::family() const noexcept -> const std::string& {
        return m_family;
    }
    auto NanFont::size() const noexcept -> float {
        return m_size_pt;
    }
    auto NanFont::weight() const noexcept -> NanFontWeight {
        return m_weight;
    }
    auto NanFont::color() const noexcept -> const NanColor& {
        return m_color;
    }
    auto NanFont::has_explicit_color() const noexcept -> bool {
        return m_has_explicit_color;
    }
    auto NanFont::letter_spacing() const noexcept -> float {
        return m_letter_spacing;
    }
    auto NanFont::overflow() const noexcept -> TextOverflow {
        return m_overflow;
    }
    auto NanFont::wrap_policy() const noexcept -> TextWrapPolicy {
        return m_wrap_policy;
    }
    auto NanFont::max_lines() const noexcept -> int {
        return m_max_lines;
    }
    auto NanFont::single_line() const noexcept -> bool {
        return m_single_line;
    }

    auto NanFont::line_height() const noexcept -> float {
        if (m_line_height_override > 0.0f)
            return m_line_height_override;
    ...
    auto NanFont::glyph_advance(std::uint32_t codepoint) const noexcept -> float {
        ...
        if (glyph_index == 0)
            return 0.0f;
        if (FT_Load_Glyph(face.ft_face, glyph_index, FT_LOAD_DEFAULT) != FT_Err_Ok)
            return 0.0f;
    ...
    namespace
    {
        ...
        auto shape_single_segment(hb_font_t* hb_font, std::string_view segment)
            -> std::vector<GlyphInfo> {
            std::vector<GlyphInfo> result;
            if (segment.empty() || !hb_font)
                return result;
                            ...
                            auto lead = static_cast<unsigned char>(segment[cluster]);
                                if (lead < 0x80) {
                                    ...
                                    original_cp = ((lead & 0x1Fu) << 6u)
                                        | (static_cast<unsigned char>(segment[cluster + 1])
                                           & 0x3Fu);
                                }
                                    ...
                                    original_cp = ((lead & 0x0Fu) << 12u)
                                        | ((static_cast<unsigned char>(segment[cluster + 1])
                                            & 0x3Fu)
                                           << 6u)
                                        | (static_cast<unsigned char>(segment[cluster + 2])
                                           & 0x3Fu);
                                }
                                    ...
                                    original_cp = ((lead & 0x07u) << 18u)
                                        ...
                                        | ((static_cast<unsigned char>(segment[cluster + 2])
                                            & 0x3Fu)
                                           << 6u)
                                        | (static_cast<unsigned char>(segment[cluster + 3])
                                           & 0x3Fu);
                                }
                        }
        ...
        auto wrap_lines(
            ...
            float ascent,
            int max_lines,
            const TextWrapPolicy wrap_policy
        ) -> std::vector<TextLine> {
                    ...
                    float space_line_w = 0.0f; // 到 last_space 为止的行宽
                        ...
                        for (std::size_t j = i; j < glyphs.size(); ++j) {
                            float adv = glyphs[j].advance_x;
                                ...
                                // Even under an extremely narrow width constraint, a wrapped line
                                // must consume at least one glyph. Otherwise line_end can remain
                                // equal to i and the outer loop never advances.
                                if (j == i && adv > max_width + k_layout_epsilon) {
                                    line_w = adv;
                                    line_end = j + 1;
                                    break;
                                }
        ...
        auto clip_glyphs(std::vector<GlyphInfo>& glyphs, float max_width)
            -> std::vector<GlyphInfo> {
            std::vector<GlyphInfo> result;
            float cursor_x = 0.0f;
                for (auto& g: glyphs) {
                    if (cursor_x + g.advance_x > max_width + k_layout_epsilon)
                        break;
        ...
        auto ellipsis_glyphs(
            ...
            float cursor_x = 0.0f;
                ...
                for (auto& g: glyphs) {
                    if (cursor_x + g.advance_x + ellipsis_w > max_width + k_layout_epsilon)
                        break;
        ...
        auto populate_line_ink_bounds(TextLine& line, const std::vector<FontStackEntry>& faces)
            -> void {
                ...
                for (const auto& glyph: line.glyphs) {
                    ...
                    const auto* bitmap = &face->glyph->bitmap;
                        if (bitmap->buffer && bitmap->width > 0 && bitmap->rows > 0) {
                            const float pen_x = cursor_x + glyph.offset_x
                                + static_cast<float>(face->glyph->bitmap_left);
                            const float pen_y = line.baseline - glyph.offset_y
    ...
    auto NanFont::shape_text(std::string_view text) const -> std::vector<std::vector<GlyphInfo>> {
        ensure_loaded();
        if (text.empty() || !m_impl || m_impl->faces.empty())
            return {};
        ...
        auto decode_utf8_cp = [](const std::string_view& str, std::size_t& idx) -> unsigned int {
            const unsigned char lead = static_cast<unsigned char>(str[idx]);
                ...
                if ((lead >> 5) == 0x6 && idx + 1 < str.size()) {
                    idx += 2;
                    return ((lead & 0x1F) << 6) | (static_cast<unsigned char>(str[idx - 1]) & 0x3F);
                }
                if ((lead >> 4) == 0xE && idx + 2 < str.size()) {
                    idx += 3;
                    return ((lead & 0x0F) << 12)
                        | ((static_cast<unsigned char>(str[idx - 2]) & 0x3F) << 6)
                        | (static_cast<unsigned char>(str[idx - 1]) & 0x3F);
                }
                if ((lead >> 3) == 0x1E && idx + 3 < str.size()) {
                    idx += 4;
                    return ((lead & 0x07) << 18)
                        | ((static_cast<unsigned char>(str[idx - 3]) & 0x3F) << 12)
                        | ((static_cast<unsigned char>(str[idx - 2]) & 0x3F) << 6)
                        | (static_cast<unsigned char>(str[idx - 1]) & 0x3F);
                }
            ++idx;
            ...
            for (const auto& segment: segments) {
                ...
                std::vector<GlyphInfo> segment_glyphs;
                    for (const auto& run: runs) {
                        ...
                        auto glyphs =
                            shape_single_segment(m_impl->faces[run.face_idx].hb_font, run_text);
                        for (auto& g: glyphs)
                            g.face_index = run.face_idx;
                        segment_glyphs.insert(
                            segment_glyphs.end(),
    ...
    auto NanFont::layout_lines(
        const std::vector<std::vector<GlyphInfo>>& segments,
        float max_width
    ) const -> TextLayout {
        if (max_width < 0.0f)
            max_width = 0.0f;
        ...
        const bool single = m_single_line;
            ...
            for (const auto& segment: segments) {
                    if (segment.empty()) {
                        ...
                        if (ml > 0 && static_cast<int>(result.lines.size()) >= ml)
                            goto done;
                        continue;
                    }
                ...
                for (auto& g: glyphs)
                    segment_total_w += g.advance_x;

                    if (single && max_width > 0.0f
                        && segment_total_w > max_width + k_layout_epsilon)
                    {
                        TextLine line;
                        line.height = lh;
                        line.baseline = asc;
                        line = ellipsis_glyphs(
                            glyphs,
                            max_width,
                            lh,
                            asc,
                                    ...
                                    case TextOverflow::wrap: {
                                        auto wrapped = wrap_lines(
                                            ...
                                            for (auto& line: wrapped) {
                                                result.lines.push_back(std::move(line));
                                                if (ml > 0
                                                    && static_cast<int>(result.lines.size()) >= ml)
                                                    goto done;
                                            }
                                        break;
                                    }
                                    ...
                                    case TextOverflow::ellipsis: {
                                        ...
                                        for (auto& g: glyphs)
                                            ...
                                            if (total_w <= max_width + k_layout_epsilon) {
                                                ...
                                                result.lines.push_back(ellipsis_glyphs(
                                                    glyphs,
                                                    max_width,
                                                    lh,
                                                    asc,
                                                    m_impl->faces.empty() ? nullptr
                                                                          : m_impl->faces[0].hb_font
                                                ));
                                            }
                                        if (ml > 0 && static_cast<int>(result.lines.size()) >= ml)
                                            goto done;
    ...
    auto NanFont::paint(
        ...
        float origin_x,
        float origin_y,
        const geometry::NanRect* clip_rect
    ) const -> void {
        ensure_loaded();
        if (!m_impl || m_impl->faces.empty() || layout.empty())
            return;
        ...
        auto get_face_for_glyph = [&](const GlyphInfo& glyph) -> FT_Face {
            return glyph.face_index < m_impl->faces.size() ? m_impl->faces[glyph.face_index].ft_face
                                                           : nullptr;
        };
                ...
                float cursor_x = origin_x;
                    for (const auto& glyph: line.glyphs) {
                        const auto face = get_face_for_glyph(glyph);
                            ...
                            if (FT_Load_Glyph(face, glyph.glyph_index, FT_LOAD_DEFAULT)
                                != FT_Err_Ok)
                            {
                                cursor_x += glyph.advance_x;
                                continue;
                        ...
                        const auto* bitmap = &face->glyph->bitmap;
                            if (bitmap->buffer && bitmap->width > 0 && bitmap->rows > 0) {
                                const float pen_x = cursor_x + glyph.offset_x
                                    + static_cast<float>(face->glyph->bitmap_left);
                                const float pen_y = std::round(
                                    line_y + line.baseline - glyph.offset_y
                                    - static_cast<float>(face->glyph->bitmap_top)
                                );
                                ...
                                float draw_y = pen_y;
                                    ...
                                    if (clip_rect) {
                                        const int clip_left =
                                            static_cast<int>(std::floor(clip_rect->x()));
                                        const int clip_top =
                                            static_cast<int>(std::floor(clip_rect->y()));
                                        const int clip_right =
                                            static_cast<int>(std::ceil(clip_rect->right()));
                                        const int clip_bottom =
                                            static_cast<int>(std::ceil(clip_rect->bottom()));

                                        const int bitmap_left =
                                            static_cast<int>(std::floor(draw_x));
                                        ...
                                        const int intersect_right =
                                            std::min(bitmap_right, clip_right);
                                        const int intersect_bottom =
                                            std::min(bitmap_bottom, clip_bottom);
                                ...
                                std::vector<uint32_t> pixels(
                                    static_cast<std::size_t>(draw_w)
                                    * static_cast<std::size_t>(draw_h)
                                );
                                    for (int row = 0; row < draw_h; ++row) {
                                            for (int col = 0; col < draw_w; ++col) {
                                                const auto alpha =
                                                    bitmap->buffer
                                                        [(row + src_y) * bitmap->pitch
                                                         + (col + src_x)];
                                                    if (alpha == 0) {
                                                        pixels
                                                        ...
                                                        uint32_t pr =
                                                            static_cast<uint32_t>(r) * alpha / 255;
                                                        uint32_t pg =
                                                            static_cast<uint32_t>(g) * alpha / 255;
                                                        uint32_t pb =
                                                            static_cast<uint32_t>(b) * alpha / 255;
                                                        uint32_t pa =
                                                            static_cast<uint32_t>(a) * alpha / 255;
                                                        pixels
                                                            [static_cast<std::size_t>(row)
                                                                 * static_cast<std::size_t>(draw_w)
                                ...
                                auto picture = tvg::Picture::gen();
                                    if (picture
                                        && picture->load(
                                               pixels.data(),
                                               static_cast<uint32_t>(draw_w),
                                               static_cast<uint32_t>(draw_h),
