...
namespace Nandina::Core::Color {
    ...
    public:
        ...
        // Named getters
        [[nodiscard]] QColor shade50() const {
            return m_shades[0];
        }
        [[nodiscard]] QColor shade100() const {
            return m_shades[1];
        }
        [[nodiscard]] QColor shade200() const {
            return m_shades[2];
        }
        [[nodiscard]] QColor shade300() const {
            return m_shades[3];
        }
        [[nodiscard]] QColor shade400() const {
            return m_shades[4];
        }
        [[nodiscard]] QColor shade500() const {
            return m_shades[5];
        }
        [[nodiscard]] QColor shade600() const {
            return m_shades[6];
        }
        [[nodiscard]] QColor shade700() const {
            return m_shades[7];
        }
        [[nodiscard]] QColor shade800() const {
            return m_shades[8];
        }
        [[nodiscard]] QColor shade900() const {
            return m_shades[9];
        }
        [[nodiscard]] QColor shade950() const {
            return m_shades[10];
        }

        // Generic access by enum index (QML dynamic / loop scenarios)
        [[nodiscard("use the returned QColor")]] Q_INVOKABLE QColor shade(int accentType) const;
    ...
    public:
        ...
        [[nodiscard("use the returned palette pointer")]] Q_INVOKABLE Nandina::Core::Color::ColorPalette *
        palette(int variantType) const;

        // Convenience: one-shot color lookup
        [[nodiscard("use the returned QColor")]] Q_INVOKABLE QColor color(int variantType, int accentType) const;
