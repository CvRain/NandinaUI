...
核心代码默认不依赖 RTTI，也不使用宏完成类型识别、API 生成或控制业务流程。预处理器只保留给包含保护、平台适配和构建系统注入的特性开关。类型识别通过显式的虚访问器完成，基类返回 `nullptr`，派生类返回自身类型，并保持 const / 非 const 对称；不得用未经验证的 `static_cast` 假定父节点类型。组件内部服务优先经 `BuildContext` 注入，窗口、页面与组件不得通过全局单例取得服务。完整规则和例外说明见 `component_contract.md` 第 8 节，新增节点类型时应回到该节逐条核对。

`app::Page` 的路由身份必须由页面显式返回稳定字符串，不能使用 `typeid(T).name()`、编译器函数签名或地址拼接生成。CI 的 GCC 主构建关闭 RTTI；新增的 `dynamic_cast` / `typeid` 会直接导致构建失败。
