...
> 本文规定组件**必须满足什么**（质量标准）。要动某个组件时**具体改哪些文件、按什么顺序**，
> 见 [新增一个组件](adding_a_component.md)。
...
核心代码默认不依赖 RTTI，也不使用宏完成类型识别或 API 生成。类型识别应通过显式的虚拟访问器（例如 `as_node2d()`、
...
API 或隐藏生命周期。宏仅用于包含保护、平台适配和构建特性开关。若确需 RTTI 或其他宏，
应在开发参考中说明原因、作用范围和未来移除路径。
