...
Text primitive 已落地为独立模块，且所有 widgets 消费面已全面迁移。

当前现实：

- `widgets/src/nan_text.cppm` 已提供独立 `Text` primitive，负责纯文本内容、TypographyRole、对齐、真实测量与绘制
- 文本底层能力仍由 `text/nan_font` 提供
- `TextField`、`Button`、`Tag`、`Card`、`Sidebar` 等所有控件内部文本节点已全面切到 `Text`
- `Label` 已退回为建立在 `Text` 之上的语义包装层，仅保留 disabled / error / required 语义
...
- 新增非语义文本消费方时，优先考虑直接复用 `Text`，不要再默认嵌入 `Label`
- 后续做 Label 收口时，应把“文本能力”和“Label 作为 control 的语义状态”继续拆开
- 目标仍然是让 `Label` 成为对 `Text` 的语义包装，而不是继续自己私有一套文本系统

一句话理解：

> 当前 Text primitive 已是完全独立的模块，整个 widgets 体系已迁移到它之上。
...
- 继续把 FocusRing 复用到后续 Checkbox 等新控件
