...
> 文档状态：已定稿（2026-05，按当前模块结构校正）
>
> 说明：本文件约束命名、导出与 API 风格；具体模块边界与实现完成度以 `docs/module-dependency-rules.md`、`docs/repository-structure.md` 与源码为准。
...
| 子命名空间 | 职能说明 | 典型导出模块 |
|----------|----------|--------------|
| `nandina::types` | 跨模块基础枚举（Axis、Align、PointerButton 等） | `nan_types` |
| `nandina::geometry` | 二维几何原语（点、尺寸、矩形、内边距、约束） | `nan_point`、`nan_size`、`nan_rect`、`nan_insets`、`nan_constraints` |
| `nandina::color` / `nandina::log` | foundation 内的颜色与日志能力 | `color`、`log` |
| `nandina::runtime` | 运行时核心（窗口、事件、Widget 树） | `nan_window`、`nan_widget`、`nan_event` |
| `nandina::reactive` | 响应式状态与订阅能力 | `state`、`effect`、`computed`、`prop` |
| `nandina::layout` | 约束、容器协议与基础布局原语 | `core`、`layout_container`、`flex_widgets` |
| `nandina::theme` | token / theme schema 与解析入口 | `theme_manager`、`nan_style_primitives` |
| `nandina::text` | 字体、shape、文本绘制与测量 | `nan_font`、`nan_text` |
| `nandina::widgets` | Button、Label、Card、Panel 等控件 | `button`、`label`、`card`、`panel` |
| `nandina::app` | authoring、页面、路由、应用窗口层 | `application`、`authoring`、`page`、`router` |
| `nandina::showcase` / `nandina::bindings` | 验证应用与未来绑定扩展 | `nan_showcase`、`bindings/*` |
